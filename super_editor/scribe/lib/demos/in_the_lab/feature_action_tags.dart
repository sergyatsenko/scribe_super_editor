import 'package:scribe/demos/in_the_lab/in_the_lab_scaffold.dart';
...
class _ActionTagsFeatureDemoState extends State<ActionTagsFeatureDemo> {
  ...
  void initState() {
    ...
    _actionTagPlugin = ActionTagsPlugin()
      ..composingActionTag.addListener(_updateActionTagList);
    ...
    setState(() {
      ...
      for (final node in _document) {
        ...
        for (final actionSpan in actionSpans) {
          _actions
              .add(node.text.substring(actionSpan.start, actionSpan.end + 1));
        }
      }
    });
  }
...
class _ActionTagsListPopoverState extends State<_ActionTagsListPopover> {
  ...
  void initState() {
    ...
    widget.actionTagPlugin.composingActionTag
        .addListener(_onComposingTokenChange);

    final initialComposingTag =
        widget.actionTagPlugin.composingActionTag.value?.tag.token;
  ...
  void didUpdateWidget(_ActionTagsListPopover oldWidget) {
    ...
    if (widget.actionTagPlugin != oldWidget.actionTagPlugin) {
      oldWidget.actionTagPlugin.composingActionTag
          .removeListener(_onComposingTokenChange);
      widget.actionTagPlugin.composingActionTag
          .addListener(_onComposingTokenChange);
    }
  }
  ...
  void _selectMatchingActions(String composingTag) {
    _matchingActions
      ..clear()
      ..addAll(_actionCandidates.where((availableAction) => availableAction.name
          .toLowerCase()
          .contains(composingTag.toLowerCase())));
  }
...
class ConvertSelectedTextNodeRequest implements EditRequest {
  ...
  bool operator ==(Object other) =>
      ...
      other is ConvertSelectedTextNodeRequest &&
    ...
    switch (newType) {
      case TextNodeType.header1:
        newNode = ParagraphNode(
          id: oldNode.id,
          text: oldNode.text,
          metadata: Map.from(oldNode.metadata)
